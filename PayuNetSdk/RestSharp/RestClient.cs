...
namespace RestSharp
{
	...
	public partial class RestClient : IRestClient
	{
		...
		public RestClient()
		{
			...
			AddHandler("application/xml", new DotNetXmlDeserializer());
		...
		private IRestResponse<T> Deserialize<T>(IRestRequest request, IRestResponse raw)
		{
            ...
            catch (Exception ex)
            {
                ...
                {
                    response.ErrorMessage = ex.Message + ". Raw content: " + raw.Content.ToString();
                }
                else
                {
        ...
        private IRestResponse<T, E> Deserialize<T, E>(IRestRequest request, IRestResponse raw)
        {
            request.OnBeforeDeserialization(raw);

            IRestResponse<T, E> response = new RestResponse<T, E>();
            try
            {
                response = raw.toAsyncResponse<T, E>();
                response.Request = request;
